...
class MyXchangeClient(xchange_client.XChangeClient):
    ...
    async def long_short_arbitrage(self):
        ...
        if (ept_ask is not None and igm_ask is not None and brv_ask is not None and scp_bid is not None):
            ...
            scp_diff = self.edge * (total_stock_cost_scp + self.swap_fee) - (scp_bid * 10)
        else:
            scp_diff = None
        ...
        if (ept_ask is not None and dlo_ask is not None and mku_ask is not None and jak_bid is not None):
            ...
            jak_diff = self.edge * (total_stock_cost_jak + self.swap_fee) - (jak_bid * 10)
        else:
            jak_diff = None

        # Returns positive values around $400 to $600
        print(f"SCP DIFF: {scp_diff}")
        print(f"JAK DIFF: {jak_diff}")
        if scp_diff is None:
            pass
        elif scp_diff < 0:
            print("short SCP")
    ...
    async def trade(self):
        ...
        for i in range(55):
            ...
            if self.positions['SCP'] >= 100:
                await self.place_swap_order('fromSCP',3)
                await asyncio.sleep(0.02)
                await self.firesale()
            await asyncio.sleep(0.02)
            if self.positions['JAK'] == 50:
                await self.place_swap_order('fromJAK', 3)
                await asyncio.sleep(0.02)
                await self.firesale()
            ...
            print(f"Estimate: {self.estimate_pnl()}")
            await asyncio.sleep(0.02)
